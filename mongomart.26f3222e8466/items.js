...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
         ...
         var skipAmount = page*itemsPerPage
        //  var pageItems = [];
        //  for (var i=0; i<5; i++) {
        //      pageItems.push(pageItem);
        //  }
        console.log(category, 'cat test');
        console.log(skipAmount,'skip')
        console.log(itemsPerPage,'ipp');
        if(category ==="All"){
          this.db.collection('item').find().sort({"_id":1})
          .skip(skipAmount).limit(itemsPerPage).toArray(function(err, items){
            console.log(items,'items 2')
            callback(items);
          })
        } else {
          this.db.collection('item').find({category: category}).sort({"_id":1})
          .skip(skipAmount).limit(itemsPerPage).toArray(function(err, items){
        ...
        //  db.item.find({category: "Apparel"}).sort({"_id":1}).skip(1).limit(2)
        //STARTING
    ...
    this.getItem = function(itemId, callback) {
         ...
         *
         */
         this.db.collection('item').find({_id: itemId}).toArray(function(err, anItem) {
             console.log(anItem[0],'rtI')
             callback(anItem[0]);
         });;
        // var item = this.createDummyItem();
